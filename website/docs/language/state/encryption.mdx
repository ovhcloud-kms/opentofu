...
This key provider uses the [OVHcloud Key Management Service](https://www.ovhcloud.com/en/identity-security-operations/key-management-service/) to generate data keys.
Authentication relies on mutual TLS (mTLS): you must provide a client certificate and a private key.
You can configure it as follows:

| Option                   | Description                                                                                                                               | Min. | Default                            |
|--------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|------|------------------------------------|
| endpoint *(required)*    | Endpoint URL. Can also be set via `TF_OKMS_ENDPOINT`.                                                                                     | N/A  | -                                  |
| key_id *(required)*      | UUID of the service key used to wrap/unwrap the data key. Can also be set via `TF_OKMS_KEY_ID`.                                           | N/A  | -                                  |
| cert *(required)*        | Path to the mTLS client certificate file. Can also be set via `TF_OKMS_CERT`.                                                             | N/A  | -                                  |
| key *(required)*         | Path to the mTLS client private key file. Can also be set via `TF_OKMS_KEY`.                                                              | N/A  | -                                  |
| ca                       | Optional path to a custom CA certificate to trust. Can also be set via `TF_OKMS_CA`.                                                      | N/A  | System CA pool                     |
| key_bits                 | Optional size of the generated data key in bits. Must be `128`, `192` or `256`.                                                           | 128  | 256                                |
| key_name                 | Optional name attached to the generated data key.                                                                                         | N/A  | -                                  |
| encrypted_metadata_alias | Optional identifier to store metadata in the encrypted state/plan files under. Specify this to allow changing the name of a key provider. | -    | derived from the key provider name |
